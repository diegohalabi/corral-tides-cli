...
def main():
    ...
    # Preparar datos del gráfico (desde las 00:00 de hoy hasta 3 días después)
    plot_start = now.replace(hour=0, minute=0, second=0, microsecond=0)
    plot_end = plot_start + datetime.timedelta(days=3)
    ...
    if not x_timestamps:
        console.print("[bold red]Error:[/bold red] No hay suficientes datos en la caché para graficar los próximos 3 días.")
        sys.exit(1)
    ...
    plt.title("Proyección a 3 días (Curva interpolada)")
    ...
    table = Table(title="Mareas Oficiales SHOA (Próximos 3 días)", expand=True, border_style="cyan")
